...
- **Released 1.8.0 (Codex, 2026-09-15).** Build 15, release commit/tag
  ...
  notarization submission `9c38f6b5-4801-4323-89d2-733ec14d344c`; stapling and
  Gatekeeper checks passed. Independent unauthenticated downloads of both public
  archives match the local release; the downloaded app's version, identifier,
  update feed, document icons, Developer ID signature, stapled ticket and
  Sparkle Ed25519 signature were verified. `glassine-appcast.xml` offers build
  15; Folio's `appcast.xml` is unchanged. The installed app was not replaced.
  Archive SHA-256:
  `3a8afb9a45404805d5bffce88451e6097a412339f96be5f962876eeb846e9e08`.
  Logs and verification files: `build/release-1.8.0/`.
  Release: https://github.com/danepps/glassine/releases/tag/v1.8.0

- **Claude review fixes (1.8.0; Codex, 2026-09-15).** File ▸ Print now uses
  `ReaderWindowController.printReaderDocument`, avoiding PDFKit's inherited
  `printDocument:` handler when the PDF view has focus. The same command owner
  validates readiness, lock state and printing permissions. Ordinary printing
  uses the original-bounds snapshot without find ink; continuous Markdown is
  repaginated. Unexpected snapshot or print-operation failures present an error.

  Highlight Markdown export escapes setext headings, separators, indented lists
  and code blocks, and preserves literal HTML entity spellings. Leading spaces
  and tabs are entity-encoded instead of discarded. The formatter's page-order
  contract and the output guard's synchronous, same-thread restriction are now
  explicit. Noncontiguous highlight deletion reselects near the first removed
  row, regardless of the order in which rows were selected.

  Reader Mode settings still save every slider change immediately. Cleanup runs
  on the first save per defaults instance and when the 500-file cap is exceeded;
  ordinary saves no longer decode every other record. A 120-save debug benchmark
  with 500 stored files improved from a 3.63 ms median to 0.69 ms. Reading and
  writing the defaults dictionary still scales with its size.

  Validation: all 76 macOS tests and 147 Core tests pass on macOS 27.0 with
  Swift 6.4. New tests resolve the real window responder chain from PDF, sidebar
  and search focus, inspect print snapshots and rendered pixels, verify actual
  WebKit pagination for continuous Markdown, exercise permission/error paths,
  parse exported Markdown, and cover deletion order and immediate settings
  persistence. The older outline-navigation fixture now assembles a fresh PDF
  page tree before adding bookmarks: standalone probing found that this PDFKit
  version can omit newly added outlines when serializing an unchanged source
  page tree. The test still serializes and reopens the fixture before navigation.

  Release build and ad-hoc signature verification pass. The isolated app's live
  PDF print preview shows all four original pages with Reader Mode and search
  active, without green find ink. The additional Markdown visual check was
  blocked by the file chooser; its native pagination regression passes. No
  physical print job was sent. Computer Use initially stalled while attaching,
  then recovered for the PDF preview.

  Logs and probes: `build/claude-review-fixes/`. Isolated app:
  `build/Glassine Claude Review Test.app`, bundle id
  `com.epps.Glassine.ClaudeReviewTest`, automatic updates disabled. The installed
  app was not replaced during validation. These fixes are included in 1.8.0.

- **Matching PDF document icon (1.8.0; Codex, 2026-09-15).** Added
  `Support/PDFDocument.icns`, generated by `scripts/make-doc-icon.swift --pdf`.
  It shares Markdown's folded sheet, coral rim and four coloured rules, with
  geometric slate PDF lettering and a single P at 16 px. `Info.plist` assigns it
  to the existing `com.adobe.pdf` document type; `build.sh` bundles both icons.
  All 10 icon entries decode at their expected sizes with preserved transparency;
  the default Markdown output remains byte-identical. The release-mode local
  build, ad-hoc signature verification and bundled-asset comparisons pass.
  Preview and validation files are in `build/document-icon-review/`. Finder's
  installed-app association was not changed; no installation or release occurred.

- **Reader Mode review follow-up (1.8.0; Codex, 2026-09-14).** Implemented
...
- **Reader Mode, low-light brightness, and Recents-only New Tab (1.8.0;
...
- **Continuous Markdown sidebar and editable sidebar search (1.8.0; Codex,
...
- **Highlight notes and Markdown export (1.8.0; Codex, 2026-09-12).**
...
swift scripts/make-doc-icon.swift --pdf   # regenerate the matching PDF icon
...
The **PDF document icon** uses the same generator with `--pdf`, writing
`Support/PDFDocument.icns`. Its slate PDF lettering is drawn as geometric paths
below the folded corner; at 16 px it simplifies to a P. The page geometry,
colours, rules, per-size tuning and 10-entry icon family match Markdown.
`CFBundleTypeIconFile` on the existing PDF document-type entry names the icon;
`build.sh` requires and copies both document icons into the bundle.
